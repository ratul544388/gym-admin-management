      ...
      <div className="flex items-center gap-3 py-4">
          ...
          <Button
            onClick={() => {
              const selectedIds = table
                .getSelectedRowModel()
                .rows.map((row) => (row.original as MembershipPlan).id);
              console.log({selectedIds})
              onOpen("deleteMembershipPlanModal", { ids: selectedIds });
            }}
            variant="destructive"
          >
            Delete
        ...
        {showEditButton && (
          <Button
            onClick={() => {
              const id = (
                table.getSelectedRowModel().rows[0].original as MembershipPlan
              ).id;
